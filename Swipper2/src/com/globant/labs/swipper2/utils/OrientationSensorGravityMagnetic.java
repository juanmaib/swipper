...
public class OrientationSensorGravityMagnetic
		implements
			SensorEventListener,
			OrientationProvider {
	...
	// private SensorEventListener m_parent; // non-null if this class should
	// call
	// its
	// parent after onSensorChanged(...) and
	// onAccuracyChanged(...) notifications
	// private Activity m_activity; // current activity for call to
	// getWindowManager().getDefaultDisplay().getRotation()

	// raw inputs from Android sensors
	private float m_Norm_Gravity; // length of raw gravity vector received in
	// onSensorChanged(...). NB: should be about 10
	private float[] m_NormGravityVector; // Normalised gravity vector, (i.e.
											// length of
	// this vector is 1), which points straight
	// up into space
	private float m_Norm_MagField; // length of raw magnetic field vector
									// received in
	// onSensorChanged(...).
	private float[] m_NormMagFieldValues; // Normalised magnetic field vector,
											// (i.e.
	...
	private float[] m_NormEastVector; // normalised cross product of raw gravity
										// vector
	// with magnetic field values, points east
	private float[] m_NormNorthVector; // Normalised vector pointing to magnetic
										// north
	// private boolean m_OrientationOK; // set true if m_azimuth_radians and
	...
	// private float m_pitch_radians; // tilt angle of the device from the
	// horizontal.
	// m_pitch_radians = 0 if the device if flat,
	// m_pitch_radians = Math.PI/2 means the device is
	// upright.
	// private float m_pitch_axis_radians; // angle which defines the axis for
	// the
	// rotation
	// m_pitch_radians

	public OrientationSensorGravityMagnetic(SensorManager sm) {
		m_sm = sm;
		// m_parent = parent;
		// m_activity = null;
	...
	public void onSensorChanged(SensorEvent evnt) {
		...
		switch (SensorType) {
			case Sensor.TYPE_GRAVITY :
				if (m_NormGravityVector == null)
					m_NormGravityVector = new float[3];
				System.arraycopy(evnt.values, 0, m_NormGravityVector, 0,
						m_NormGravityVector.length);
				m_Norm_Gravity = (float) Math.sqrt(m_NormGravityVector[0]
						* m_NormGravityVector[0] + m_NormGravityVector[1]
						* m_NormGravityVector[1] + m_NormGravityVector[2]
			...
			case Sensor.TYPE_MAGNETIC_FIELD :
				if (m_NormMagFieldValues == null)
					m_NormMagFieldValues = new float[3];
				System.arraycopy(evnt.values, 0, m_NormMagFieldValues, 0,
						m_NormMagFieldValues.length);
				m_Norm_MagField = (float) Math.sqrt(m_NormMagFieldValues[0]
						* m_NormMagFieldValues[0] + m_NormMagFieldValues[1]
						* m_NormMagFieldValues[1] + m_NormMagFieldValues[2]
		...
		if (m_NormGravityVector != null && m_NormMagFieldValues != null) {
			...
			float norm_East = (float) Math.sqrt(East_x * East_x + East_y
					* East_y + East_z * East_z);
			if (m_Norm_Gravity * m_Norm_MagField * norm_East < 0.1f) { // Typical
																		// values
																		// are >
																		// 100.
				// m_OrientationOK = false; // device is close to free fall (or
				// in
				...
				float M_dot_G = (m_NormGravityVector[0]
						* m_NormMagFieldValues[0] + m_NormGravityVector[1]
						* m_NormMagFieldValues[1] + m_NormGravityVector[2]
						* m_NormMagFieldValues[2]);
				float North_x = m_NormMagFieldValues[0]
						- m_NormGravityVector[0] * M_dot_G;
				float North_y = m_NormMagFieldValues[1]
						- m_NormGravityVector[1] * M_dot_G;
				float North_z = m_NormMagFieldValues[2]
						- m_NormGravityVector[2] * M_dot_G;
				float norm_North = (float) Math.sqrt(North_x * North_x
						+ North_y * North_y + North_z * North_z);
				...
				// rotation
				// int rotation =
				// m_activity.getWindowManager().getDefaultDisplay().getRotation();
				// float screen_adjustment = 0;
				float screen_adjustment = (float) Math.PI / 2;
				// switch (rotation) {
				...
				float sin = m_NormEastVector[1] - m_NormNorthVector[0], cos = m_NormEastVector[0]
						+ m_NormNorthVector[1];
				m_azimuth_radians = (float) (sin != 0 && cos != 0 ? Math.atan2(
						sin, cos) : 0);
				...
				// float aximuth_plus_two_pitch_axis_radians = (float) (sin != 0
				// && cos != 0 ? Math.atan2(sin, cos) : 0);
				// m_pitch_axis_radians = (float)
				// (aximuth_plus_two_pitch_axis_radians - m_azimuth_radians) /
				// 2;
	...
	public double getAzimuthDegrees() {
		return Math.toDegrees(m_azimuth_radians);
	}
